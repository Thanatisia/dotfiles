...
"======================
let fs=[
    \ vimhome . '/tmp/documents/'
    \ ]

" Create default/extra Filesystem Structure (Files/Folders) if doesnt exist
for item in fs
    " Check if folder exists
    if !isdirectory(item)
        " If directory does not exist
        " make directory
        echo "Initial Creation of folder" . ' ' . item . ' ' . "Created."
        call mkdir(item, "p")
    endif
endfor
...
"   + curl
"   + git
" - Check if Vim is 'nvim': If is nvim, plugin data directory is 'nvim_data_dir/site/autoload/plug.vim', Else if is pure vim: plugin data directory is '~/.vim/autoload/plug.vim
let plugin_data_dir = has('nvim') ? stdpath('data') . '/site' : '~/.vim' 
" Automatic download of Plugin Manager
if empty(glob(plugin_data_dir . '/autoload/plug.vim'))
    " If plug.vim doesnt exist
    " - Download
    silent execute '!curl -fLo' . ' ' . plugin_data_dir . '/autoload/plug.vim --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim'
...
" Syntax : Plug 'author/project', { 'key' : 'value' }
    ...
    Plug 'mhinz/vim-startify' " Provides a start screen for vim/nvim on initial startup without a file
    " Expansion Plugin
...
"======================="
let ext_Files = [
    \ g:vimcfg . '/configs/local/functions.vim',
    \ g:vimcfg . '/configs/local/commands.vim' ,
    \ g:vimcfg . '/configs/local/autorun.vim'
    \ ]
" Loop all external files created and source them
for ext_File in ext_Files
    " Check if current file exists
    if filereadable(ext_File)
        " If file exists
        " source the file
        execute 'source ' . ext_File
    else
        echo ext_File . ' is not sourced'
    endif
endfor
...
" Important
    ...
    " Re-source VimRC
        silent nnoremap <expr> <leader>rvimrc ':so ' . vimcfg . '/' . vimname . '<CR>'
    " Refresh File
        nnoremap <leader><F5> :e!<CR>
" Help
    " Check variables for currently set values based on keyword
        nnoremap <expr> <leader>?env ':echo' . ' ' . '$' . input("Environment Variable: ") . "<CR>"
        nnoremap <expr> <leader>?sys ':echo' . ' ' . '&' . input("System Variable: ") . "<CR>"
        nnoremap <expr> <leader>?set ':set' . ' ' . input("Settings to check: ") . '?' . "<CR>"
    " Validation
        nnoremap <expr> <leader>?val ':verbose' . ' ' . input('Type: ') . ' ' . input('Keyword: ') . "<CR>" 
" Bug-Fixing
" Quality of Life
    " Wrap selected texts in Quotation Marks '"'
        vnoremap " <esc>`>a"<esc>`<i"<esc>
    " Clear last search highlighting
        nnoremap <leader>csh :noh<CR>
...
" File Type-specific Remap
    augroup filetype 
        ...
        autocmd FileType python 
            \ execute 'nnoremap <buffer> ' . key_Run . ' :w<CR>:!python "%" '
